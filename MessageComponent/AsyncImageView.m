...
- (void)loadFailedWithError:(NSError *)error
{
	self.loading = NO;
	self.cancelled = NO;
    //yok dusmuyoruz cunku loading NO ancak ekranda yıne loadıgn var.
...
- (void)setImageURL:(NSURL *)imageURL
{
    ...
    if (self.showActivityIndicator && !self.image && imageURL)
    {
        if (self.activityView == nil)
        {
          ...
          //load ekranı fotoların burası g94n2man
           LoadingView*_loading=[[LoadingView alloc]init];
            LoadingImageView=[_loading Imageloading:self.activityView];
            [self.activityView addSubview:LoadingImageView];
        }
